...
public abstract class Person implements java.io.Serializable {
	...
	{
		if (DOB.getYear() < (java.time.LocalDate.now().getYear() - 100) ) {
			throw new PersonException("Person too old",this);
		}
		
		String regex = "^\\(?([0-9]{3})\\)?[-.\\s]?([0-9]{3})[-.\\s]?([0-9]{4})$";
		
		if (!(
				Pattern.compile(regex).matcher(Phone_number).matches()
				)) {
			throw new PersonException("Phone number in improper format",this);
		}
